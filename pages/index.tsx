      ...
      {contact && (
        <div id="contact">
          <div className={styles.contactContainer}>
            <h2 className={styles.experienceHeading}>{contact.sectionTitle}</h2>
            <p>{contact.sectionDescription}</p>
            {contact?.social && contact?.social?.map((social) => (
              <a className={styles.social__links} key={social.name} href={social.link} target="_blank" rel="noreferrer" title={social.name}>
